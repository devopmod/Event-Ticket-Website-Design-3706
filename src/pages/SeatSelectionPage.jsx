...
const { FiArrowLeft, FiCalendar, FiMapPin, FiClock, FiAlertCircle, FiTrash2 } = FiIcons;
...
const SeatSelectionPage = () => {
  ...
  const handleSeatSelect = async (seat) => {
    const isSelected = selectedSeats.find(s => s.id === seat.id || (s.elementId && s.elementId === seat.elementId));
    ...
    if (isSelected) {
      ...
      try {
        if (seat.elementId) {
          ...
          const seatsToRemove = selectedSeats.filter(s => s.elementId === seat.elementId);
          for (const seatToRemove of seatsToRemove) {
            if (seatToRemove.id && !seatToRemove.id.includes('-' + Date.now())) {
              await releaseSeats(id, [seatToRemove.id]);
            }
          }
      ...
      try {
        if (seat.elementId) {
          // For capacity-based seats, just add to local selection
          ...
          // For regular seats, hold them on the server
  ...
  const handleRemoveSeat = async (seatToRemove) => {
    try {
      if (seatToRemove.elementId) {
        // For capacity-based seats, remove all seats from this element
        const seatsToRemove = selectedSeats.filter(s => s.elementId === seatToRemove.elementId);
        for (const seat of seatsToRemove) {
          if (seat.id && !seat.id.includes('-' + Date.now())) {
            await releaseSeats(id, [seat.id]);
          }
        }
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="max-h-48 overflow-auto space-y-2">
                      {selectedSeats.map((seat, index) => (
                        <div
                          key={`${seat.id || seat.elementId}-${index}`}
