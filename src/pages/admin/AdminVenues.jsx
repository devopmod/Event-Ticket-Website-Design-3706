...
import {fetchVenues, createVenue, updateVenue, deleteVenue, getVenueSeatsCount} from '../../services/venueService';
import {compareVenueSeatCounts, generateSeatDiscrepancyNotification} from '../../services/venueVerificationService';
import {regenerateSeatsForVenue} from '../../services/eventService';

const {FiPlus, FiSearch, FiEdit, FiTrash2, FiMapPin, FiUsers, FiX, FiGrid, FiCheckCircle, FiAlertTriangle} = FiIcons;
...
const AdminVenues = () => {
  ...
  const handleSaveVenue = async (venueData, venueId) => {
    setSaving(true);
    setError('');
    try {
      let result;
      ...
      if (venueId) {
        ...
        result = await updateVenue(venueId, venueData);
      } else {
        // Create new venue
      ...
      if (result) {
        ...
        setVenues(prev => 
          venueId 
            ? prev.map(v => v.id === venueId ? result : v) 
            : [result, ...prev]
        );

        // Update venue stats
        const venueSeatsCount = await getVenueSeatsCount(result.id);
        setVenueStats(prev => ({...prev, [result.id]: venueSeatsCount}));
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              ...
              >
                <SafeIcon icon={FiPlus} className="mr-2" /> New Venue
              </button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mb-6">
                {searchTerm ? 'Try adjusting your search terms' : 'Create your first venue to get started with seating arrangements'}
              </p>
              {!searchTerm && (
                <button
                  onClick={handleCreateVenue}
