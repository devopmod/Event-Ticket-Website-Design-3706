  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (isResizing && selectedElement && elementStart) {
      ...
      if (selectedElement.type === 'seat') {
        ...
        // Horizontal changes
        if (position.includes('w')) { // Left side
          newX = elementStart.x + dx;
          newWidth = Math.max(20, elementStart.width - dx);
        } else if (position.includes('e')) { // Right side
          newWidth = Math.max(20, elementStart.width + dx);
        }
        
        // Vertical changes
        if (position.includes('n')) { // Top side
          newY = elementStart.y + dy;
          newHeight = Math.max(20, elementStart.height - dy);
        } else if (position.includes('s')) { // Bottom side
          newHeight = Math.max(20, elementStart.height + dy);
        }
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-zinc-800/90 text-white p-3 rounded-lg text-sm max-w-xs">
        <div className="space-y-1">
          ...
          {selectedTool === 'polygon' && (
            <div className="text-primary-400"><strong>Polygon Tool:</strong> Click to add points, double-click to finish</div>
          )}
          {selectedTool === 'select' && (
            <div className="text-primary-400"><strong>Select Tool:</strong> Drag to move, handles to resize</div>
          )}
