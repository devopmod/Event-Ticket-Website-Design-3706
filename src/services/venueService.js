...
export const getVenueSeatsCount=async (venueId)=> {
  try {
    ...
    if (error || !venue) {
      ...
      return {total: 0, byType: {seat: 0, section: 0, polygon: 0}, byCategory: {}, elements: 0};
    }
    ...
    return {total: 0, byType: {seat: 0, section: 0, polygon: 0}, byCategory: {}, elements: 0, bookableElements: 0};
  }
};
...
export const updateVenue=async (id, venueData)=> {
  try {
    ...
    // Ensure we're using the original venue ID provided in the parameter
    const updatePayload = {
      ...venueData,
      updated_at: new Date().toISOString()
    };
    ...
    // Update the venue with the specified ID
    const {data, error}=await supabase
      .from(VENUES_TABLE)
